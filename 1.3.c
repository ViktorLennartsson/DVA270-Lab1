...
void read_string(char *inArray)
{
	...
	do
	{
		...
		while (!nrfx_uarte_rx_ready(&instance)); //<- kan behövas //Väntar på att motta tecken
...
{
	char outputBuffer[MAXSIZE+1];   //Skapar char sträng
	sprintf(outputBuffer, "%d", inputNumber);   //Konverterar inputNumber till string och lägger den i outputBuffer
